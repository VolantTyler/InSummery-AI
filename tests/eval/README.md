...
This directory contains the evaluation harness for the two LLM-backed agents in
the InSummery workflow: the **triager** (registration / disruption / general
...
The harness evaluates the *exact* agents used in production: both
`app/nodes.py` and the eval runner build agents from the shared factories in
`app/agent_factories.py`, and eval inputs go through the same PII
mask → model → unmask round-trip as real traffic.

## Running the evals

```bash
# Run all evals, gate on absolute thresholds and the stored baseline
insummery-eval run
